...
class Lowerer:
    ...
    def _lower_select_expr(self, ex: ast.SelectExpr, hf: HIRFunction) -> str:
        res = self._tmp()
        l_end = self._tmp()
        for idx, c in enumerate(ex.cases):
            l_case = self._tmp()
            hf.instrs.append(HIRInstr("label", l_case, None, None, "void"))
            if c.kind == "recv" and c.channel:
                ch = self._lower_expr(c.channel, hf)
                val = self._tmp()
                hf.instrs.append(HIRInstr("call.recv", val, ch, "1", ex.inferred_type or "i32"))
                hf.instrs.append(HIRInstr("mov.i32", res, val, None, ex.inferred_type or "i32"))
                ...
                hf.instrs.append(HIRInstr("mov.i32", res, v, None, ex.inferred_type or "i32"))
            else:
                hf.instrs.append(HIRInstr("const.i32", res, "0", None, ex.inferred_type or "i32"))
            for s in c.body.stmts:
                self._lower_stmt(s, hf)
            hf.instrs.append(HIRInstr("jmp", l_end, None, None, "void"))
            if idx < len(ex.cases) - 1:
                hf.instrs.append(HIRInstr("label", self._tmp(), None, None, "void"))
        hf.instrs.append(HIRInstr("label", l_end, None, None, "void"))
        return res
